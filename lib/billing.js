...
async function cancelSubscription(userId, subscriptionId) {
    ...
    const subscription = await _1.stripe.subscriptions.update(subscriptionId, { cancel_at_period_end: true });
    if (subscription.status === 'canceled') {
        await firebase_1.db
            .collection('users')
            .doc(userId)
            .update({
            activePlans: firebase_admin_1.firestore.FieldValue.arrayRemove(subscriptionId),
        });
    }
    return subscription;
}
